...
public class Shader implements Runnable{
		...
		while(true){
			...
			for(int j = VBOsStart; j < VBOsEnd; j++) {
		  		...
		  		for(int i = 0; i < VBO.triangleCount; i++) {
		  			...
		  			//用索引缓冲来构建三角形
		  			updatedVertices[0] = VBO.updatedVertexBuffer[VBO.indexBuffer[i*3+2]];
		  			updatedVertices[1] = VBO.updatedVertexBuffer[VBO.indexBuffer[i*3+1]];
		  			updatedVertices[2] = VBO.updatedVertexBuffer[VBO.indexBuffer[i*3]];
	...
	//测试隐藏面
  	...
  	//找出两点之间的线段在裁剪平面的交点
  	public  void approximatePoint(int index, Vector3D behindPoint, Vector3D frontPoint){
  ...
  //给三角形的像素着色
  	...
  	//画单色的三角形
  	public  void rendersolidTriangle() {
  		//逐行渲染扫描线
