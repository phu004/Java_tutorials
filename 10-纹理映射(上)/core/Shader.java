...
public class Shader extends Thread{
	...
	public Vector3D[] clippedVertices;
  	...
  	//辅助渲染计算纹理的变量
  	public Vector3D uDirection, vDirection, origin, point;
				...
				global_cosX = LookupTables.cos[360-Camera.X_angle];
		  		...
		  		for(int i = 0; i < VBO.triangleCount; i++) {
			  		...
			  		if(VBO.renderType == VertexBufferObject.textured) {
			  			...
			  			uDirection.set(VBO.UVDirections[firstVertex][0]).scale(VBO.textureScaleX / VBO.scale);
			  			vDirection.set(VBO.UVDirections[firstVertex][1]).scale(VBO.textureScaleY / VBO.scale);
	...
	//测试隐藏面
  	...
  	public  void scanTriangle() {
		...
		if(VBO.renderType == VertexBufferObject.soildColor)
			...
			rendersolidTriangle_zDepth_shading_textured();
	...
	public void handleClipping(){
  	...
  	//画有纹理的三角形 （填充深度值，使用亮度）
  	public void rendersolidTriangle_zDepth_shading_textured() {
		    ...
		    float light_right = lightRight[i];
  			...
  			for(int j = x_left;  j < x_right; j++, z_Left+=dz, lightLevel+=dlight){
  				if(zBuffer[j] < z_Left) {         //如果深度浅于深度缓冲上的值
  					...
  					float WorldY = (float)(half_screen_h - i) / screenDistance  * WorldZ;
  					
  					point.set(WorldX, WorldY, WorldZ);
  					point.subtract(origin);
  					
  					int X = (int)(uDirection.dot(point)*width);
  					int Y = (int)(vDirection.dot(point)*height);
  					
  					int textureIndex = (X&widthMask) + ((Y&heightMask)<<widthBits);
